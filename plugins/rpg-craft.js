    ...
    const buttons1 = [
      {
        ...
        buttonText: { displayText: 'Craft 🎣FishingRod' },
        type: 1,
      },
      {
        buttonId: `${prefix}craft pickaxe`,
        buttonText: { displayText: 'Craft ⛏️Pickaxe' },
        type: 1,
      },
      {
        buttonId: `${prefix}craft sword`,
        buttonText: { displayText: 'Craft ⚔️Sword' },
        type: 1,
      },
    ]
    ...
    switch (type) {
      case 'fishingrod':
        if (user.fishingrod * 1 > 0) {
          const buttons = [
            {
              buttonId: `${prefix}upgrade fishingrod`,
              buttonText: { displayText: 'Upgrade 🎣FishingRod' },
              type: 1,
            },
          ]
...
untuk mengupgrade ketik *${usedPrefix}upgrade fishingrod*`.trim()
      ...
      case 'pickaxe':
        if (user.pickaxe * 1 > 0) {
          const buttons = [
            {
              buttonId: `${prefix}upgrade pickaxe`,
              buttonText: { displayText: 'Upgrade ⛏️Pickaxe' },
              type: 1,
            },
          ]
...
${batu < 30 ? `\n🪨Batu kamu kurang *${30 - batu}*` : ''}${kayu < 20 ? `\n🪵Kayu kamu kurang *${20 - kayu}*` : ''}${money < 15000 ? `\n💵Uang kamu kurang *${15000 - money}*` : ''}`)
      ...
      case 'sword':
        if (user.sword * 1 > 0) {
          const buttons = [
            {
              buttonId: `${prefix}upgrade sword`,
              buttonText: { displayText: 'Upgrade ⚔️Sword' },
              type: 1,
            },
          ]
