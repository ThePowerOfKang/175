...
handler.before = async function (m) {
  ...
  if (!(id in this.asahotak)) return this.sendButton(m.chat, 'Soal itu telah berakhir', wm, 'Asah Otak', '.asahotak', m)
  if (m.quoted.id == this.asahotak[id][0].id) {
    ...
    if (m.text.toLowerCase() == json.jawaban.toLowerCase()) {
      ...
      this.sendButton(m.chat, benar + ` +${this.asahotak[id][2]} XP`, wm, 'Lagi?', '.asahotak', m)
