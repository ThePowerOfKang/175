...
`.trim()
        conn.send2Button(m.chat, str, wm, 'menu', usedPrefix + 'menu', 'inventory', usedPrefix + 'inv', m)
        if (mythic > 0) {
          global.db.data.users[m.sender].mythic += mythic * 1
          conn.send2Button(m.chat, '*Selamat anda mendapatkan item Rare yaitu*\n' + mythic + `${rpg.emoticon('mythic')}Mythic Crate`, wm, 'menu', usedPrefix + 'menu', 'inventory', usedPrefix + 'inv', m)
        }
        if (legendary > 0) {
          global.db.data.users[m.sender].legendary += legendary * 1
          conn.send2Button(m.chat, '*Selamat anda mendapatkan item Epic yaitu*\n' + legendary + `${rpg.emoticon('legendary')}Legendary Crate`, wm, 'menu', usedPrefix + 'menu', 'inventory', usedPrefix + 'inv', m)
        }
