let handler = async (m, { conn, command, args, usedPrefix }) => {
  try {
    ...
    const buttons1 = [
      {
        buttonId: `${prefix}craft fishingrod`,
        buttonText: {
          displayText: `Craft ${rpg.emoticon('fishingrod')}FishingRod`,
        },
        type: 1,
      },
      {
        buttonId: `${prefix}craft pickaxe`,
        buttonText: { displayText: `Craft ${rpg.emoticon('pickaxe')}Pickaxe` },
        type: 1,
      },
      {
        buttonId: `${prefix}craft sword`,
        buttonText: { displayText: `Craft ${rpg.emoticon('sword')}Sword` },
        type: 1,
      },
    ]
    ...
    switch (type) {
      case 'fishingrod':
        if (fishingrod == 0) {
          const buttons = [
            {
              buttonId: usedPrefix + `craft fishingrod`,
              buttonText: {
                displayText: `Craft ${rpg.emoticon('fishingrod')}FishingRod`,
              },
              type: 1,
...
untuk mendapatkannya ketik *${usedPrefix}craft fishingrod*`
      ...
      case 'pickaxe':
        if (pickaxe == 0) {
          const buttons = [
            {
              buttonId: usedPrefix + `craft pickaxe`,
              buttonText: {
                displayText: `Craft ${rpg.emoticon('pickaxe')}Pickaxe`,
              },
              type: 1,
            },
          ]
...
${batu < __batu ? `\n${rpg.emoticon('batu')}Batu kamu kurang *${__batu - batu}*` : ''}${kayu < __kayu ? `\n${rpg.emoticon('kayu')}Kayu kamu kurang *${__kayu - kayu}*` : ''}${money < __money ? `\n${rpg.emoticon('money')}Uang kamu kurang *${__money - money}*` : ''}`,
      ...
      case 'sword':
        if (sword == 0) {
          const buttons = [
            {
              buttonId: usedPrefix + `craft sword`,
              buttonText: {
                displayText: `Craft ${rpg.emoticon('sword')}sword`,
              },
              type: 1,
            },
          ]
